...
# new approach: for modularity, pass objects handling separate steps of the 
# regression with their own params

# note: in the future it might be cleaner to implement things like method, 
# iter_direction, etc. as enums
# also, not a good idea to use backward-forward=false with combinatorial start 
# as it will stop at max_k

# rich regression output object
class RegressionResult(object):
    def __init__(
        self, xi, lambd, best_term, lambda1, all_xis=None, all_lambdas=None, 
        ...
        # reminder: row i of these arrays corresponds to i+1 term model
        self.all_xis = all_xis
        self.all_lambdas = all_lambdas
        self.all_lambdas_test = all_lambdas_test
        # list of actual terms corresponding to this sublibrary - can also be set later
        self.sublibrary = sublibrary

# pre- and postprocessing by scaling/nondimensionalizing data
class Scaler(object):
    def __init__(
        self, sub_inds, char_sizes, row_norms=None, unit_rows=False, 
        train_fraction=1
    ):
        # note: char_sizes should always be set
        self.full_w = len(char_sizes)
        # default is keeping all indices
        self.sub_inds = (sub_inds if sub_inds is not None 
                         else list(range(self.full_w)))
        self.w = len(self.sub_inds)
        # useful if we want to reuse a Scaler except with different sub_inds
        # self.full_cs = (np.array(char_sizes) if char_sizes is not None 
        #                 else np.ones(shape=(self.w,)))
        #self.char_sizes = self.full_cs[self.sub_inds]
        self.char_sizes = np.array(char_sizes)
        self.row_norms = (np.array(row_norms) if row_norms is not None 
                          else None)
    ...
    def index(self, col):
        # find the full indexing to sub_inds conversion of a given column number
        return self.sub_inds.index(col) if col is not None else None
    
    def norm_col(self, theta, col):
        # compute norm of given column after nondimensionalization
    ...
    #         theta[:, term] /= self.char_sizes[term]
    ...
    def postprocess_multi_term(
        self, xi, lambd, test_train_ratio=None, lambda_test=None, verbose=False
    ):
        # Xi postprocessing
        ...
        return (full_xi, lambd, 
                lambda_test/np.sqrt(test_train_ratio) if lambda_test else None) 
        #/norm # already normalized in multi-term regression
        
    def postprocess_single_term(
        self, best_term, lambda1, test_train_ratio=None, lambda1_test=None, 
        verbose=False
    ):
        if verbose:
            print("final lambda1:", lambda1)
        return (self.sub_inds[best_term], lambda1, 
                lambda1_test/np.sqrt(test_train_ratio) if lambda1_test else None)
    
    def __repr__(self):
        return (f"Scaler(sub_inds={self.sub_inds}, char_sizes={self.char_sizes}, "
                f"row_norms={self.row_norms})")
...
class Initializer(object): # selecting initial guess
    ...
    def make_model(self, theta, verbose):
        if self.method == 'combinatorial':
            # note - can also be used to initialize iteration from full library 
            # using start_k>=w
            w = theta.shape[1]
            inds = list(range(w))
            best_lambd = np.inf
            combos = (combinations(inds, self.start_k) 
                      if self.start_k < len(inds) else (inds,))
            ...
            xi, mu, it = TInvPower(
                sigma_in, self.start_k, mu0=None, verbose=False, 
                forced_col=self.inhomog_col
            )
            if verbose:
                print("mu:", mu, ", # of iterations: ", it)
            # always return absolute residual until postprocessing
...
class ModelIterator(object): # selecting next iterate and enforcing stopping condition
    def __init__(
        self, max_k, backward_forward=True, max_passes=10, use_best_solution=True
    ):
        # threshold, brute_force=True
        ...
        # sadly non-brute force seems to drop key terms in dominant balance
        self.inhomog = None
        # note: should be set to converted index in sublibrary
        self.inhomog_col = None
        ...
        # True if backward-forward iteration has already turned around
        self.state = None # most recently saved set of xis
        # self.stopped_early = False 
        # may want to set True if early stopping so selection is clear
        # to prevent infinite loop from ever occuring
    ...
    def __repr__(self):
        return (f"ModelIterator(max_k={self.max_k}, "
                f"backward_forward={self.backward_forward}, "
                f"max_passes={self.max_passes}, inhomog={self.inhomog}, "
                f"inhomog_col={self.inhomog_col}, k={self.k}, "
                f"direction={self.direction}, terms={self.terms}, "
    ...
    def save_state(self, xis, lambdas, test_lambdas, verbose):
        # set the internal state & update best solutions found so far
        self.state = np.copy(xis)
        # no values have been set yet or we are just saving last state
        if self.best_state is None or not self.use_best_solution:
            if verbose:
            ...
            for i in range(xis.shape[0]): # update each row individually
                # better solution has been found for this sparsity
                if self.best_lambdas[i]>lambdas[i]:
                    if verbose:
                        print(f"Best lambda for k={i+1}: "
                              f"{self.best_lambdas[i]} -> {lambdas[i]}")
    ...
    def drop(self, theta, xi, verbose, represent):
        # term with lowest score will be dropped
        s = np.zeros(shape=(len(self.terms), 1))
        for i, ind in enumerate(self.terms):
            if ind == self.inhomog_col:
                ...
                if self.inhomog:
                    ...
                    s[i] = (np.linalg.norm(theta @ xi) / 
                            np.abs(xi[self.inhomog_col]))
                else:
                ...
                #             col -= (np.dot(col, other_col) / 
            #                     np.linalg.norm(other_col)**2 * other_col)
                #     s[i] = np.linalg.norm(xi[ind] * col)
        best = self.terms[np.argmin(s)]
        if verbose:
            print("Scores of terms to remove:", [
                (represent(i), float(j)) for i, j in zip(self.terms, s)
            ])
    ...
    def pick(self, theta, xi, verbose, represent):
        w = theta.shape[1]
        # term with lowest score will be dropped
        s = np.zeros(shape=(w-len(self.terms), 1))
        ...
        for i, ind in enumerate(other_terms):
            ...
            #     col = theta[:, ind]
            #     proj = residual_col - (np.dot(residual_col, col) / 
            #                             np.linalg.norm(col)**2 * col)
            #     s[i] = -np.linalg.norm(proj) 
            #     # guess for xi[col] not yet available 
        best = other_terms[np.argmin(s)]
        if verbose:
            print("Scores of terms to add:", [
                (represent(i), float(j)) for i, j in zip(other_terms, s)
            ])
            print("Adding term:", represent(best))
        return best
    
    def check_exit(self, xis, lambdas, test_lambdas, verbose):
        # we haven't finished iterating to one of the ends of the pass range
        if self.k!=1 and self.k!=self.max_k:
            # it is possible to add early stopping conditions on lambdas here
            return False
        ...
        # update iterator's saved solutions
        self.save_state(xis, lambdas, test_lambdas, verbose)
        # entries in xis have changed via iteration
        if self.backward_forward and np.any(differences):
            self.passes += 1
            ...
            if verbose:
                ...
                print(f"{self.passes} passes, continuing "
                      f"backward-forward iteration...")
            return False
        if verbose:
            print("Iteration completed.")
        return True # otherwise we are done
        # for instance suppose xis did not change during b or f iteration. when it 
        # was saved it was optimal going in the other direction, and currently is 
        # optimal in the other direction, so no update will occur.
...
class Residual(object): # residual computation
    ...
    # residual_type "matrix_relative" is relative to norm of theta[:, sub_inds] 
    # after preprocessing
    # residual_type "hybrid" uses "matrix_relative" for regression and then 
    # theta[:, support] for reporting
    # residual_type "dominant_balance" is relative to norm of largest term in 
    # initial dominant balance
    ...
    def __repr__(self):
        return (f"Residual(type={self.residual_type}, norm={self.norm}, "
                f"anchor_col={self.anchor_col})")
        
class Threshold(object):
    def __init__(
        self, threshold_type, delta=1e-10, gamma=1.5, epsilon=1e-2, 
        ic=None, n_terms=None
    ):
    ...
    def __repr__(self):
        return (f"Threshold(type={self.type}, delta={self.delta}, "
                f"gamma={self.gamma}, epsilon={self.epsilon}, "
                f"ic={self.information_criterion}, n_terms={self.n_terms})")
    ...
    def select_model(self, lambdas, theta, lambda1, verbose):
        ...
        if self.type == 'jump': # check when lambda>delta and jump in lambda>gamma
            ...
            i = len(lambdas)-1
            while ((jumps[i-1]<self.gamma or lambdas[i-1]<self.delta) 
                   and i>0):
                i -= 1
            if verbose:
                print("Jumps:", jumps)
            ...
            h = theta.shape[0]
            ics = [
                self.information_criterion(lambd, i+1, h) 
            ...
            while lambdas[i]/lambda1>self.epsilon and i<len(lambdas):
                i += 1
            return i
        # select model before largest residual increase - don't use this one
...
def sparse_reg_bf(
    theta, scaler, initializer, residual, model_iterator, threshold, 
    inhomog=False, inhomog_col=None, full_regression=False, 
    term_names=None, verbose=False, seed=1
):
    ...
    lambda1_test = (np.linalg.norm(theta_test[:, best_term]) 
                    if h_test>0 else None)
    # won't work for dominant balance
    if residual.residual_type in [
        "matrix_relative", "hybrid", "fixed_column", "absolute"
    ]:
        lambda1 = lambda1 / residual.norm
        lambda1_test = (lambda1_test / residual.norm 
                        if h_test>0 else None)

    # HANDLE W=0 (inf), W=1 (one-term model) CASES
    if w == 0:  # no inds allowed at all
        return RegressionResult(
            xi=None, lambd=np.inf, best_term=None, lambda1=np.inf, 
            sublibrary=term_names
        )
    if w == 1:  # no regression to run
        #norm = residual.norm if hasattr(residual, 'norm') else 1
        best_term, lambda1, lambda1_test = scaler.postprocess_single_term(
            best_term=best_term, lambda1=lambda1,
            test_train_ratio=test_train_ratio, lambda1_test=lambda1_test,
            verbose=verbose
        )
        return RegressionResult(
            xi=[1], lambd=np.inf, best_term=best_term, lambda1=lambda1, 
    ...
    # any lambdas that are not computed are assumed to be very large
    lambdas = np.inf*np.ones(shape=(max_k,))
    test_lambdas = np.inf*np.ones(shape=(max_k,))
    # we have reversed order of arrays compared to the old SR script - index = n_terms-1
    xis[k-1, :] = xi
    ...
    if not full_regression:
        ...
        # NOTE that the indices are wrt sublibrary!
        model_iterator.set_terms(w_inds[xi!=0], verbose)
        ...
        test_lambdas[k-1] = (np.linalg.norm(theta_test @ xi)/residual.norm 
                             if h_test>0 else None)

        # initial support is smaller than the maximum -> the initial guess works 
        # for smaller k's as well
        if actual_k<k:
        ...
        # save initial state
        model_iterator.save_state(xis, lambdas, test_lambdas, verbose)
        exit = False
        while not exit:
            ### ITERATION RULE
            ...
            #print("TESTING: K=", k)
            # we have reversed order of arrays compared to old SR - index = n_terms-1
            xis[k-1, :] = xi
            # WE NORMALIZE LAMBDA HERE FOR CONSISTENT EXIT CRITERION
            lambdas[k-1] = lambd/residual.norm
            test_lambdas[k-1] = (np.linalg.norm(theta_test @ xi)/residual.norm 
                                 if h_test>0 else None)
        ...
        # we now use best solution found so far
        ind = threshold.select_model(
            model_iterator.best_lambdas, theta, lambda1, verbose
        )
    ...
    # use best solutions!
    xi, lambd = model_iterator.best_state[ind, :], model_iterator.best_lambdas[ind]
    lambda_test = (model_iterator.best_test_lambdas[ind] 
                   if h_test>0 else None)

    # switch residual from matrix_relative to hybrid for output
    if residual.residual_type == "hybrid":
    ...
    # model is normalized so coeff of b column is 1 instead of unit norm, 
    # which impacts the residuals -> we fix that here
    if inhomog:
        xi_norm = np.linalg.norm(xi)
        lambd, lambda1 = lambd/xi_norm, lambda1/xi_norm
        lambda_test, lambda1_test = (
            (lambda_test/xi_norm, lambda1_test/xi_norm) 
            if h_test>0 else (None, None)
        )
    xi, lambd, lambda_test = scaler.postprocess_multi_term(
        xi=xi, lambd=lambd, test_train_ratio=test_train_ratio, 
    ...
    # return RegressionResult(xi=xi, lambd=lambd, best_term=best_term, lambda1=lambda1, 
    #                        all_xis=xis, all_lambdas=lambdas, lambda_test=lambda_test, 
    #                        all_lambdas_test=test_lambdas, lambda1_test=lambda1_test, 
    #                        sublibrary=term_names)
    return RegressionResult(
        xi=xi, lambd=lambd, best_term=best_term, lambda1=lambda1, 
        all_xis=model_iterator.best_state, all_lambdas=model_iterator.best_lambdas,
        lambda_test=lambda_test, all_lambdas_test=model_iterator.best_test_lambdas, 
        lambda1_test=lambda1_test, sublibrary=term_names
    )

# evaluate specific model on Q 
# NOTE: use scaler with same column subsampling as any models being compared with 
# & make sure model_vector sparsity matches it
def evaluate_model(theta, model_vector, scaler, residual, verbose=False):
    ...
    model_vector, lambd, _ = scaler.postprocess_multi_term(
        xi=model_vector, 
        lambd=np.linalg.norm(theta @ model_vector)/residual.norm, 
        verbose=verbose
    )
    return lambd

def hybrid_residual(theta, xi, scaler, return_xi=False):
    # compute the "hybrid" residual for an existing model
    # preprocessing
    theta = scaler.scale_theta(theta)   
    theta = scaler.select_cols(theta) 
    ...
    if return_xi:
        # xi, lambd, _ = scaler.postprocess_multi_term(
        #     xi, np.linalg.norm(theta @ xi)/norm)
